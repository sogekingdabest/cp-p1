...
void *swap(void *ptr)
{
	...
	while(args->iterations--) {
		...
		do{
			...
			pthread_mutex_lock(&(args->lmutex[i]));

			if(pthread_mutex_trylock(&(args->lmutex[j]))){

                		pthread_mutex_unlock(&(args->lmutex[i]));
                		parejaBloqueada = 0;
		...
		pthread_mutex_unlock(&(args->lmutex[i]));
		pthread_mutex_unlock(&(args->lmutex[j]));
...
void start_threads(struct options opt)
{
	...
	for(i=0; i<buffer.size; i++){
        	...
        	if(pthread_mutex_init(&lmutex[i],NULL)){
            		printf("No se pudieron inicializar el mutex %d\n", i);
            		exit(1);            
        	}
    	}
